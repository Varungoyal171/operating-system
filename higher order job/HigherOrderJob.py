# -*- coding: utf-8 -*-
"""
Created on Sun Sep 13 15:25:33 2020
...
while(j<i):
    ...
    c=float(input("Enter Priority (Higher No.-Higher Priority): "))
    arv.append(a)
    burst.append(b)
    priority.append(c)
...
headers=["Programs","Arival Time","Burst time ","Priority"]
table=zip(array,arv,burst,priority)
...
while(each<time):
    ...
    for every in range(i):
        if(arv[every]<=each):
            ...
            priority2.append(priority[every])
    
    if(len(end)>0):
        z=priority2.index(max(priority2)) #smallest job program out of list
